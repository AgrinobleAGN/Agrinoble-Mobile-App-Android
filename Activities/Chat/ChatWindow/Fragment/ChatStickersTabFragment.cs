...
{
    public class ChatStickersTabFragment : AndroidX.Fragment.App.Fragment, TabLayoutMediator.ITabConfigurationStrategy
    {

        private TabLayout Tabs;
        private readonly string Type;
        public ChatStickersTabFragment(string type = "ChatWindowActivity")
        {
            Type = type;
        }
        ...
        private void SetUpViewPager(ViewPager2 viewPager)
        {
            try
            {
                ...
                if (AppSettings.ShowStickerStack0)
                    adapter.AddFragment(new StickerFragment1(Type), "0");

                if (AppSettings.ShowStickerStack1)
                    adapter.AddFragment(new StickerFragment2(Type), "1");

                if (AppSettings.ShowStickerStack2)
                    adapter.AddFragment(new StickerFragment3(Type), "2");

                if (AppSettings.ShowStickerStack3)
                    adapter.AddFragment(new StickerFragment4(Type), "3");

                if (AppSettings.ShowStickerStack4)
                    adapter.AddFragment(new StickerFragment5(Type), "4");

                if (AppSettings.ShowStickerStack5)
                    adapter.AddFragment(new StickerFragment6(Type), "5");

                if (AppSettings.ShowStickerStack6)
                    adapter.AddFragment(new StickerFragment7(Type), "6");
