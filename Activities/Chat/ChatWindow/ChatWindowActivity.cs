...
namespace WoWonder.Activities.Chat.ChatWindow
{
    ...
    public class ChatWindowActivity : AppCompatActivity, MaterialDialog.IListCallback, View.IOnLayoutChangeListener, IOnRecordClickListener, IOnRecordListener, SwipeReply.ISwipeControllerActions
    {
        ...
        public string UserId, ChatId, ReplyId; // to_id
        ...
        private void SuggestionAdapterOnItemClick(object sender, AdapterClickEvents e)
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                {
                    ...
                    {
                        UserDetails.Socket?.EmitAsync_SendMessage(UserId, UserDetails.AccessToken, UserDetails.Username, SuggestionAdapter.GetItem(position).RealMessage, MainChatColor, false, time2);
                    }
                    else
                    {
            ...
            try
            {
                ...
                else if (RecordButton?.Tag?.ToString() == "Text")
                {
                    ...
                    else
                    {
                        ...
                        if (SelectedItemPositions?.MesData != null && !string.IsNullOrEmpty(ReplyId))
                        {
                            m1.ReplyId = ReplyId;
                        ...
                        if (Methods.CheckConnectivity())
                        {
                            ...
                            {
                                UserDetails.Socket?.EmitAsync_SendMessage(UserId, UserDetails.AccessToken, UserDetails.Username, EmojIconEditTextView.Text, MainChatColor, false, time2);
                            }
                            else
                            {
                                Task.Factory.StartNew(() =>
