...
namespace WoWonder.SocketSystem
{
	public class AppNotificationsManager
	{
		...
		public void ShowUserNotification(Context mContext, string conversationId, string username, string message, string userId, string ChatId, string avatar, string color, int counterUnreadMessages = 1)
		{
			try
			{
				string channelId;
				...
				Intent messagingIntent = new Intent(Application.Context, typeof(MsgTabbedMainActivity));
				...
				//messagingIntent.PutExtra("PageId", PageId);
				//messagingIntent.PutExtra("GroupId", GroupId);
				messagingIntent.PutExtra("type", "user");
